...
const ScribeProposedData = ({navigation,route}) => {
      ...
      return focus
       ...
       const ProviderDecline=async(data)=>{
        ...
        const ProviderAccept=async(data)=>{
          ...
          const Tab0Url=URL.ProposedUrl+`${Scribe}/${ID}?startdate=${firstday}&enddate=${firstday}`
          const Tab1Url=URL.ProposedUrl+`${Scribe}/${ID}?startdate=${tomorrow}&enddate=${tomorrow}`
          const Tab2Url=URL.ProposedUrl+`${Scribe}/${ID}?startdate=${firstday}&enddate=${lastday}`
          const Tab3Url=URL.ProposedUrl+`${Scribe}/${ID}?startdate=${firstday}`
          const Tab4Url=URL.ProposedUrl+`${Scribe}/${ID}?startdate=${selectedDate}&enddate=${selectedDate}`
    
      
       const ProposedReferalList = async()=>{
       ...
       const LocationTypes='&locationtype=Facility'
        ...
        // setRowcount(result?.Rowcount);
         ...
         const url=URL.ProposedUrl+`${Scribe}/${ID}?startdate=${firstday}`;
         let result=await fetch(url);
         result=await result.json();
   ...
   const sortedData=Proposed?.sort((a,b)=>{
    ...
    const SlotView = ({ item }) => {
        return (
            ...
            <TouchableHighlight
            onPress={() => navigation.navigate('ProposedDetailView',{ID,Scribe,item,VisitTypeId:'1000'})}
            style={{ 
...
{Tab=== 0 &&(
        ...
        />
       {((!selecteddata)&&((Proposed?.length!=0)||(Rowcount===0)))&&(
...
export default ScribeProposedData;
