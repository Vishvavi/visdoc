...
const FacilityPropose = ({ navigation,route }) => {
    const{ID,AppointmentId,Scribe}=route.params;

    useEffect(()=>{
       console.log(AppointmentId)
    },[])

    const convertTime12to24 = (hourtime12) => {
        const [times, modifier] = hourtime12.split(' ');
    ...
    const ToProvider = () => {
            ...
            setselectDate1(false)   
                    ...
                    const AssignedTime=`${Hour.hour}:${Minutes.min} ${Am}`
                    if(AssignedDate&&Hour&&Minutes&&Am!=null){
                    navigation.navigate('Provider',{ID,Scribe,LocationTypeId,FacilityId,patient,specialty,TabName,VisitTypeId,AssignedDate,AssignedTime,ReferringProvider})
                }
                else
                {
                    console.log('false')  
...
<Calendar1 onSelectDate={setSelectedDate} 
 ...
 <View style={{}}>
    ...
    <View style={{marginTop:0,padding:responsiveWidth(5),marginLeft:0}}>
            ...
            <TouchableOpacity onPress={ProviderPropose} 
...
export default FacilityPropose;
