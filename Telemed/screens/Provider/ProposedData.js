...
const ProposedData = ({navigation,route}) => {
  ...
  useDimensionsChange(
   ...
   tomorrow = moment(tomorrow).add(1, 'day').format('YYYY-MM-DD');
      ...
      useEffect(()=>{
       ...
       const Tab0Url=URL.ProposedUrl+`Provider/${ID}?startdate=${moment(new Date()).format("YYYY-MM-DD")}&enddate=${moment(new Date()).format("YYYY-MM-DD")}`
      const Tab1Url= URL.ProposedUrl+`Provider/${ID}?startdate=${tomorrow}&enddate=${tomorrow}`
      const Tab2Url=URL.ProposedUrl+`Provider/${ID}?startdate=${firstday}&enddate=${lastday}`
      const Tab3Url=URL.ProposedUrl+`Provider/${ID}?startdate=${firstday}`
      const Tab4Url=URL.ProposedUrl+`Provider/${ID}?startdate=${selectedDate}&enddate=${selectedDate}`
  ...
  setProposed(result?.ListOfRefferals);
   ...
   const sortedData=Proposed?.sort((a,b)=>{
    ...
    const renderHiddenItem = (data, rowMap) => (
        <View style={styles.rowBack}>
          ...
          closeRow(rowMap,data.item.AppointmentId)}
            ...
            style={[styles.backRightBtn, {right:75}]}
          // onPress={() => closeRow(rowMap, data.item.AppointmentId)}
          >
            <Text style={{color:'black',fontSize:12,fontFamily:'paceGrotesk-Regular'}}>Propose</Text>
          </TouchableOpacity>
    ...
    const SlotView = ({ item }) => {
        return (
            <TouchableHighlight
            onPress={() => navigation.navigate('ProposedDetailView',{ID,Scribe,item,VisitTypeId:'1000',Usercreated:UserCreated})}
            style={{ 
