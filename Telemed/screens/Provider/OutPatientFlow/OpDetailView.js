...
const OpDetailView = ({route,navigation}) => {
    ...
    const ItemView= (item) => {
        return(
           ...
           <View style={{flexDirection:'row'}}>
            ...
            <View style={{}}>
            <Text style={{fontSize:22,width:responsiveWidth(65),borderWidth:0,color:'#333333',fontFamily: 'SpaceGrotesk-Medium',marginLeft:10}}>{item.PatientName}</Text>
         ...
         <Text style={{fontSize:12,color:'#333333',fontFamily:'SpaceGrotesk-Regular',marginLeft:10}}>{item.LocationTypeName}-{item.HomeHealthCompanyName}</Text>
           ...
           { //<Text style={{fontSize:12,color:'#333333',fontFamily: 'SpaceGrotesk-Regular'}}>{moment(item.AppointmentDate).format("MMM' D.YYYY")}</Text>
    }
           </View>
