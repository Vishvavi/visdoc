import { SafeAreaView, ScrollView, StyleSheet, Text, View,RefreshControl, Image, TouchableOpacity, Alert, FlatList } from 'react-native'
...
// get provider availability list

    const getListAvailability = async() => {
        try{
        const url=`https://visdocapidev.azurewebsites.net/api/ProviderAvailability/Date/${ID}`
...
const Inactivedate=(inactivedate,inactiveSTime,inactiveETime)=>{
  
 const data=
    {
        providerId: `${ID}`,
          Date: `${inactivedate}`,
            StartTime: `${inactiveSTime}`,
            EndTime: `${inactiveETime}`
          }
      
      console.log(data)
    const url = "https://visdocapidev.azurewebsites.net/api/ProviderAvailability/Date/Inactive"
    fetch(url, {
   ...
   <View style={{flexDirection:'row',padding:20,justifyContent:'space-between'}}>
                ...
                <Text style={{ color: 'black', fontFamily:'SpaceGrotesk-Medium',width:responsiveWidth(20),fontSize:12,marginTop:0 }}>Date</Text>
                <Text style={{fontSize:15,color: 'black',}}>{item?.Date}</Text>
             </View>
           ...
           var updateDate=item.Date;
           var startTimes=item.StartTime;
           var endTime=item.EndTime;
           navigation.navigate("AvailabilityUpdate",{updateDate,startTime:startTimes,Tab:1,ID,endTime:endTime})
                ...
                <TouchableOpacity onPress={()=>{
                    var inactivedate=item.Date
                    var inactiveSTime=item.StartTime
                    var inactiveETime=item.EndTime
                    Inactivedate(inactivedate,inactiveSTime,inactiveETime)}}
                 style={{ marginTop: 10,marginLeft:20,alignItems:'center',justifyContent:'center', width: 100, height: 40, backgroundColor: '#EAEAEA', borderRadius: 8 }}>
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={{flexDirection:'row',alignItems:'center',borderWidth:0}}>
              <TouchableOpacity onPress={()=>navigation.goBack()}>
                ...
                <TouchableOpacity style={{marginRight:responsiveWidth(5)}} onPress={()=>navigation.navigate('AvailabilityUpdate',{updateDate:'',startTime:`${moment(new Date()).format('HH:mm:ss')}`,endTime:`${moment(new Date()).format('HH:mm:ss')}`,Tab:0,ID}) }>
