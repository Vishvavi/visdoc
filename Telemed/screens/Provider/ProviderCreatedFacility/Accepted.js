import { FlatList, Image, ScrollView, StyleSheet, Text, TouchableOpacity,RefreshControl,Animated,Keyboard, View,LayoutAnimation } from 'react-native'
...
const AcceptedforFcreated = ({navigation,route}) => {
...
const Tab0Url=URL.createdFAcceptedUrl+`${ID}?startdate=${moment(new Date()).format("YYYY-MM-DD")}&enddate=${moment(new Date()).format("YYYY-MM-DD")}`
const Tab1Url=URL.createdFAcceptedUrl+`${ID}?startdate=${tomorrow}&enddate=${tomorrow}`
const Tab2Url=URL.createdFAcceptedUrl+`${ID}?startdate=${firstday}&enddate=${lastday}`
const Tab3Url=URL.createdFAcceptedUrl+`${ID}?startdate=${firstday}`
const Tab4Url=URL.createdFAcceptedUrl+`${ID}?startdate=${selectedDate}&enddate=${selectedDate}`
    ...
    const SlotView = ({ item }) => {
        return (
            ...
            <View style={[styles.Itemcontainer,{flex:1,marginLeft:responsiveWidth(5),marginRight:responsiveWidth(5)}]}>
                ...
                <TouchableOpacity onPress={()=>navigation.navigate('DetailView',{item,ID:ID,VisitTypeId:"1001",Scribe,Usercreated:true})}>
                ...
                {item?.LocationTypeName==='Facility'&&(
                   ...
                   <View style={{flexDirection:'row'}}>
                    <Text style={[List.Text,{width:responsiveWidth(30)}]}>{tConvert(item.AppointmentTime)}</Text>
...
{Tab=== 0 &&(
        ...
        {((!selecteddata)&&(Rowcount!=false))&&(
            ...
            <Text  style={{color:'red',fontFamily: 'SpaceGrotesk-Bold',fontSize:12}}>Cancelled/Rescheduled</Text>
            </Animated.View>
         )}
            </View>
    </View>
...
export default AcceptedforFcreated;
