...
const CreatedFacilityDeclinedRef = ({navigation,route}) => {
  ...
  const{Tab,ID,selectedDate,Scribe}=route.params;
    ...
    const Tab0Url=URL.createdFDeclinedUrl+`${ID}?startdate=${firstday}&enddate=${firstday}`
    const Tab1Url=URL.createdFDeclinedUrl+`${ID}?startdate=${tomorrow}&enddate=${tomorrow}`
    const Tab2Url=URL.createdFDeclinedUrl+`${ID}?startdate=${firstday}&enddate=${lastday}`
    const Tab3Url=URL.createdFDeclinedUrl+`${ID}?startdate=${firstday}`
    const Tab4Url=URL.createdFDeclinedUrl+`${ID}?startdate=${selectedDate}&enddate=${selectedDate}`

      const DeclinedReferalList = async(Usercreated)=>{
        if(Tab===0){
         try{
               const url=Tab0Url;
                console.log(url);
...
const onRefresh =() => {
  ...
  setTimeout(() => {
    ...
    const SlotView = ({ item }) => {
        return (
            <TouchableHighlight
            onPress={() =>navigation.navigate('DeclinedDetailView',{ID,Scribe,VisitTypeId:'1001',item,Usercreated:true})}
...
export default CreatedFacilityDeclinedRef;
