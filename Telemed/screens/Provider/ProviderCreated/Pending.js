...
const Pendingforcreated = ({navigation,route}) => {
...
const CurrentTime=`${moment(new Date()).format('HH:mm:ss')}`
      ...
      const Tab0Url=URL.createdPendingUrl+`${ID}?startdate=${firstday}&enddate=${firstday}`
      const Tab1Url=URL.createdPendingUrl+`${ID}?startdate=${tomorrow}&enddate=${tomorrow}`
      const Tab2Url=URL.createdPendingUrl+`${ID}?startdate=${firstday}&enddate=${lastday}`
      const Tab3Url=URL.createdPendingUrl+`${ID}?startdate=${firstday}`
      const Tab4Url=URL.createdPendingUrl+`${ID}?startdate=${selectedDate}&enddate=${selectedDate}`
...
<Text style={{color:'white',fontSize:12,fontFamily:'SpaceGrotesk-Regular'}}>Cancel</Text>
    ...
    const SlotView = ({item} ) => {
        ...
        return (
            ...
            <TouchableHighlight
            onPress={() => navigation.navigate('PendingDetailView',{ID,Scribe,item,VisitTypeId:"1000",Usercreated:true
            })}
            style={{ 
...
export default Pendingforcreated;
