...
const CreatedDeclinedList = ({navigation,route}) => {
  ...
  const{Tab,ID,selectedDate,Scribe}=route.params;
    ...
    tomorrow = moment(tomorrow).add(1, 'day').format('YYYY-MM-DD');
      ...
      const Tab0Url=URL.createdDeclinedUrl+`${ID}?startdate=${firstday}&enddate=${firstday}`
      const Tab1Url=URL.createdDeclinedUrl+`${ID}?startdate=${tomorrow}&enddate=${tomorrow}`
      const Tab2Url=URL.createdDeclinedUrl+`${ID}?startdate=${firstday}&enddate=${lastday}`
      const Tab3Url=URL.createdDeclinedUrl+`${ID}?startdate=${firstday}`
      const Tab4Url=URL.createdDeclinedUrl+`${ID}?startdate=${selectedDate}&enddate=${selectedDate}`
...
const xOffset = new Animated.Value(0);
    ...
    const SlotView = ({ item }) => {
        return (
            <TouchableHighlight
            onPress={() => navigation.navigate('DeclinedDetailView',{ID,Scribe,VisitTypeId:'1000',item,Usercreated:true})}
...
export default CreatedDeclinedList;
