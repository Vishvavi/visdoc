...
const Acceptedforcreated = ({navigation,route}) => {
...
const Tab0Url= URL.createdAcceptedUrl+`${ID}?startdate=${firstday}&enddate=${firstday}`
const Tab1Url= URL.createdAcceptedUrl+`${ID}?startdate=${tomorrow}&enddate=${tomorrow}`
const Tab2Url= URL.createdAcceptedUrl+`${ID}?startdate=${firstday}&enddate=${lastday}`
const Tab3Url= URL.createdAcceptedUrl+`${ID}?startdate=${firstday}`
const Tab4Url= URL.createdAcceptedUrl+`${ID}?startdate=${selectedDate}&enddate=${selectedDate}`
    ...
    const SlotView = ({ item }) => {
        return (
            ...
            <View style={[styles.Itemcontainer,{flex:1,marginLeft:responsiveWidth(5),marginRight:responsiveWidth(5)}]}>
                ...
                <TouchableOpacity onPress={()=>navigation.navigate('DetailView',{item,ID:ID,VisitTypeId:"1000",Scribe,Usercreated:true})}>
...
export default Acceptedforcreated;
